...
package com.freetech.vpn.logic;
...
import com.freetech.vpn.R;
import com.freetech.vpn.data.VpnProfile;
import com.freetech.vpn.data.VpnProfile.SelectedAppsHandling;
import com.freetech.vpn.data.VpnType.VpnTypeFeature;
import com.freetech.vpn.logic.VpnStateService.ErrorState;
...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener {
    ...
    public static final String DISCONNECT_ACTION = "com.freetech.vpn.android.CharonVpnService.DISCONNECT";
    private static final String NOTIFICATION_CHANNEL = "com.freetech.vpn.android.CharonVpnService.VPN_STATE_NOTIFICATION";
...
//                    profile.setName(bundle.getString("ZoneName"));
...
//                    profile.setCertificateString(bundle.getString("PrivateKey"));
//                    profile.setSelectedAppsHandling(0);
//                    //added by ybbk
//                    String selectApps = intent.getStringExtra("selectApps");
//                    if (selectApps != null && selectApps.trim().length() > 0) {
//                        profile.setSelectedApps(selectApps.trim());
...
//                        mCurrentUserCertificateAlias = mCurrentProfile.getUserCertificateAlias();
    ...
    private Notification buildNotification(boolean publicVersion) {
        ...
        Intent intent = new Intent("com.base.vpn.home.hero").addCategory("android.intent.category.DEFAULT");
        PendingIntent pendingIntent = PendingIntent.getActivity(this, 1009, intent, FLAG_MUTABLE);

        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, NOTIFICATION_CHANNEL)
                .setSmallIcon(R.drawable.ic_vpn_state_success)
                .setCategory(NotificationCompat.CATEGORY_SERVICE)
                .setContentIntent(pendingIntent)
                .setVisibility(publicVersion ? NotificationCompat.VISIBILITY_PUBLIC
                        : NotificationCompat.VISIBILITY_PRIVATE);
        String s = getResources().getString(R.string.vpn_state_disabled);
        if (error != ErrorState.NO_ERROR) {
            s = mService.getErrorText();
            builder.setSmallIcon(R.drawable.ic_vpn_state_error);
            builder.setColor(ContextCompat.getColor(this, R.color.vpn_state_error));

            if (!publicVersion && profile != null) {
                int retry = mService.getRetryIn();
                if (retry > 0) {
                    builder.setContentText(getResources().getQuantityString(R.plurals.vpn_retry_in, retry, retry));
...
//                builder.addAction(R.drawable.ic_notification_connecting, getString(R.string.retry), pending);
            ...
            switch (state) {
                case CONNECTING:
                    s = getResources().getString(R.string.vpn_state_connecting);
                    builder.setSmallIcon(R.drawable.ic_vpn_state_connecting);
                    builder.setColor(ContextCompat.getColor(this, R.color.vpn_state_connecting));
                    add_action = true;
                    break;
                case CONNECTED:
                    s = getResources().getString(R.string.vpn_state_connected);
                    builder.setColor(ContextCompat.getColor(this, R.color.vpn_state_success));
                    builder.setUsesChronometer(true);
                    add_action = true;
                    break;
                case DISCONNECTING:
                    s = getResources().getString(R.string.vpn_state_disconnecting);
                    break;
            }
        }
...
//        builder.setContentIntent(pending);
    ...
    private X509Certificate getX509CertificateFromString(String x509String) throws CertificateException {
        InputStream inputStream = new ByteArrayInputStream(x509String.getBytes());
...
//        return KeyChain.getPrivateKey(getApplicationContext(), mCurrentUserCertificateAlias);
    ...
    public class BuilderCache {
        ...
        public BuilderCache(VpnProfile profile) {
            ...
            if (profile.getDnsServers() != null) {
                for (String server : profile.getDnsServers().split("\\s+")) {
                    try {
                        mDnsServers.add(IPUtils.parseInetAddress(server));
        ...
        public void addDnsServer(String address) {
            ...
            try {
                mDnsServers.add(IPUtils.parseInetAddress(address));
                recordAddressFamily(address);
            } catch (UnknownHostException e) {
                e.printStackTrace();
            }
        ...
        private boolean isIPv6(String address) throws UnknownHostException {
            InetAddress addr = IPUtils.parseInetAddress(address);
    ...
    public native static byte[] parseInetAddressBytes(String address);
}
